...
const ReservationsScreen = () => {
  ...
  return (
    <div className="max-w-[1000px] m-auto">
      ...
      <div className="flex flex-row flex-wrap gap-5 md:flex-nowrap md:gap-0 md:flex-col w-full">
      {reservation.reservations?.map((info) => (
        <div key={info.id} className="flex md:flex-row md:w-full w-[310px]  flex-col items-center justify-between m-auto shadow-md rounded-lg mb-4 md:m-5 md:h-[80px] p-2">
          ...
          <div className="flex flex-col justify-center">
            <h1 className="text-md font-bold">{
             info.car.daily_rate * (new Date(info.due_date).getTime() - new Date(info.reservation_date).getTime()) / (1000 * 3600 * 24)
            }$ </h1>
